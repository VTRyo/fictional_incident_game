...
### ゲーム仕様
1. ****
   - このゲームはワークショップを進行するためのイントロダクション的立ち位置
   - ゲームマスターとインシデント対応者:kenとyukoの会話
   - 声の音声はあとで付けられるようにしておく

2. plot.mdを読み込んで

### 画像素材管理
**重要**: 画像が不足している場合は、該当箇所にコメントで以下の形式で記載してください：

```
; TODO: 画像素材が必要
; 検索キーワード: "サーバールーム 緊急事態"
; ファイル名: server_room_emergency.jpg
; 用途: インシデント発生時の背景画像
```
...
- 変数名は意味のある英語名を使用
- コメントは日本語で詳細に記述
- 画像素材が不足している場合は、必ずコメントで検索キーワードを明記し、あとで素材収集できるようにしてください
